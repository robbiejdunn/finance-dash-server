...
```bash
finance-dash-server$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
finance-dash-server$ sam local start-api
finance-dash-server$ curl http://localhost:3000/
```
...
```bash
finance-dash-server$ sam logs -n HelloWorldFunction --stack-name finance-dash-server --tail
```
...
```bash
finance-dash-server$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name finance-dash-server
```
